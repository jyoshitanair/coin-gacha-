...
export default function Library({uuid}) {
    ...
    function handleClick(id){
        const {data, error} = await supabase.from("flashcards").select('*').eq("id", uuid).order("id", {ascending:false})
    }
                    ...
                    <div className = "grid_container">
                        {currentPulls && currentPulls.map((pull) => (
                            <div  key = {pull.id} className = "grid_item">
                                    ...
                                    <button onClick = {() => handleClick(pull.id)} type = "button"> Click to practice </button>
                            </div>))}
                    </div>
                    <button 
                    className = "buttons_normal" 
