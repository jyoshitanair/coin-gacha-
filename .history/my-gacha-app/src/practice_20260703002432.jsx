    ...
    return(
        <div>
            ...
            {mode == "test" && <div>
                ...
                <button disabled = {index == item.flashcardData.length - 1} onClick = {() => {setIndex(prev => (Math.min(item.flashcardData.length-1,prev +1)))}}> correct</button>
                <button disabled = {index == 0} onClick = {() => {setIndex(prev => (Math.max(0,prev -1)))}}> wrong</button>
