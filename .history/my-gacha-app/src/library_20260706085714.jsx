...
export default function Library({uuid}) {
    ...
    useEffect(() => {
        ...
        if (data){
            data.map((pull) => {
                if(pull.title.length > 305){
                pull.title = pull.title.slice(0, 305) + "...";
                }
                if(pull.description.length > 16){
                    pull.description = pull.title.slice(0, 16) + "..."
                }
            })
            setPulls(data)
        }
                    ...
                    <div className = "grid_container">
                        {currentPulls && currentPulls.map((pull) => (
                            <div  key = {pull.id} className = "grid_item2">
                                    ...
                                    <button disabled = {processing} onClick = {() => handleClick(pull.id)} type = "button"> Click to practice </button>
                            </div>))}
                    </div>
                    <button 
                    disabled = {processing}
