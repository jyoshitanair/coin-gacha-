...
export default function Library({uuid}) {
    ...
    useEffect(() => {
        ...
        if (data){
            data.map((pull) => {
                if(pull.title.length > 24){
                pull.title = pull.title.slice(0, 24) + "...";
                }
                if(pull.description.length > 13){
                    pull.description = pull.title.slice(0, 13) + "..."
                }
            })
            setPulls(data)
        }
                    ...
                    <div className = "grid_container">
                        {currentPulls && currentPulls.map((pull) => (
                            <div  key = {pull.id} className = "grid_item2">
                                    <p className = "smallp">Title: {pull.title}</p>
                                    <p className = "smallp">Description: {pull.description}</p>
                                    <button  className = "buttons_normal"  disabled = {processing} onClick = {() => handleClick(pull.id)} type = "button"> Click to practice </button>
                            </div>))}
                    </div>
                    <button 
                    disabled = {processing}
