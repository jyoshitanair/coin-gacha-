        ...
        {fail == false && page == "practice" &&
            <>
                ...
                {mode == "test" && <div>
                    ...
                    <button className = "buttons_normal" disabled = {(index == item.flashcardData.length - 1) || loading || !buttonText} onClick = {() => {updateCoinCount(true)}}> correct</button>
                    <button className = "buttons_normal" disabled = {(index == item.flashcardData.length - 1) || loading || !buttonText} onClick = {() => {setIndex(prev => (Math.min(item.flashcardData.length-1,prev +1)))}}> wrong</button>
