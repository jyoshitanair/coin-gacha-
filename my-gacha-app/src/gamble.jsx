...
export default function Flashcards() {
    ...
    function percentCalc(points1,percent1,amount) {
        ...
        // 1-5, 1-3,1
        return Math.floor(Math.random() *(final-start+1))+start
    }
    function createChar() {
        var points = 0
        var newChar = {}
       ...
       Object.keys(assets).forEach((category) => {
            ...
            if((category == "body") || (category == "head") || (category == "hair")){
                index = percentCalc(points,percent,9)
            }
            //these have 11 !
            if((category == "accessories")){
                index = percentCalc(points,percent,11)
            }
            //these have 20 !
            if((category == "eyes") || (category == "mouths")){
                index = percentCalc(points,percent,20)
            }

            newChar[category] = index
       });
       var rarity = (Math.floor((points - 18)/6)*5)+1
       if (rarity >= 5){
            rarity = 5
       }
       if (rarity <= 0){
